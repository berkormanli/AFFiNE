...
import type { SprintViewData } from './define.js';

export class SprintSingleView extends SingleViewBase<SprintViewData> {
  ...
  sprintDuration$ = computed(() => {
    return this.data$.value?.sprintDuration ?? 14;
  });

  startDate$ = computed(() => {
    return this.data$.value?.startDate;
  });

  get columns(): SprintColumn[] {
    return this.propertiesRaw$.value.filter(property => !property.hide$.value);
  }
  ...
  get sprintDuration() {
    return this.view?.sprintDuration ?? 14;
  }
  ...
  getHeaderCover(_rowId: string): SprintColumn | undefined {
  ...
  getHeaderIcon(_rowId: string): SprintColumn | undefined {
  ...
  getHeaderTitle(_rowId: string): SprintColumn | undefined {
  ...
  setSprintDuration(duration: number) {
    this.dataUpdate(() => {
      return {
  ...
  propertyGetOrCreate(columnId: string): SprintColumn {
    return new SprintColumn(this, columnId);
  }
}

type SprintColumnData = SprintViewData['columns'][number];
export class SprintColumn extends PropertyBase {
  override move(position: InsertToPosition): void {
    this.sprintView.dataUpdate(view => {
  ...
  viewData$ = computed(() => {
    return this.sprintView.data$.value?.columns.find(v => v.id === this.id);
  });

  viewDataUpdate(
    updater: (viewData: SprintColumnData) => Partial<SprintColumnData>
  ): void {
    this.sprintView.dataUpdate(data => {
      return {
        ...data,
  ...
  constructor(
    private readonly sprintView: SprintSingleView,
    columnId: string
  ) {
    super(sprintView, columnId);
  }
}
