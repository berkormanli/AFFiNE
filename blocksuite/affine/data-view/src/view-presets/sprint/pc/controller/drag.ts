...
export class SprintDragController implements ReactiveController {
  dragStart = (ele: SprintCard, evt: PointerEvent) => {
    ...
    const currentGroup = ele.closest('affine-data-view-sprint-group');
  ...
  getInsertPosition = (
    ...
    | { group: SprintGroup; card?: SprintCard; position: InsertToPosition }
    | undefined => {
    const eles = document.elementsFromPoint(evt.x, evt.y);
    const target = eles.find(v => v instanceof SprintGroup) as SprintGroup;
    if (target) {
  ...
  showIndicator = (
    evt: MouseEvent,
    self: SprintCard | undefined
  ): { group: SprintGroup; position: InsertToPosition } | undefined => {
  ...
  constructor(private readonly logic: SprintViewUILogic) {}
      ...
      this.host.disposables.add(
        this.logic.handleEvent('dragStart', context => {
          ...
          if (target instanceof Element) {
            const cell = target.closest('affine-data-view-sprint-cell');
            if (cell?.isEditing$.value) {
              return;
            }
            cell?.selectCurrentCell(false);
            const card = target.closest('affine-data-view-sprint-card');
            if (card) {
...
const createDragPreview = (card: SprintCard, x: number, y: number) => {
  const preOpacity = card.style.opacity;
  card.style.opacity = '0.5';
  const div = document.createElement('div');
  const sprintCard = new SprintCard();
  sprintCard.cardId = card.cardId;
  sprintCard.sprintViewLogic = card.sprintViewLogic;
  sprintCard.isFocus$.value = true;
  sprintCard.style.backgroundColor = 'var(--affine-background-primary-color)';
  div.append(sprintCard);
