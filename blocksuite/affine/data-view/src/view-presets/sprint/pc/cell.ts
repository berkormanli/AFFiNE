...
import type { SprintViewSelection } from '../selection';
...
const styles = css`
  ...
  affine-data-view-sprint-cell:hover {
    background-color: var(--affine-hover-color);
  }

  affine-data-view-sprint-cell .icon {
    display: flex;
    align-items: center;
  ...
  affine-data-view-sprint-cell .icon svg {
    width: 16px;
    height: 16px;
...
export class SprintCell extends SignalWatcher(
  ...
  selectCurrentCell = (editing: boolean) => {
    const selectionView = this.sprintViewLogic.selectionController;
    if (selectionView) {
  ...
  override connectedCallback() {
    ...
    this._disposables.addFromEvent(this, 'click', e => {
      ...
      e.stopPropagation();
      const selectionElement = this.sprintViewLogic.selectionController;
  ...
  isSelected(selection: SprintViewSelection) {
    if (
...
declare global {
  interface HTMLElementTagNameMap {
    'affine-data-view-sprint-cell': SprintCell;
  }
}
