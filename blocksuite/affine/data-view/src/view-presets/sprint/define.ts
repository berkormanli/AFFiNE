...
import { SprintSingleView } from './sprint-view-manager.js';

export const sprintViewType = viewType('sprint');

export type SprintViewColumn = {
  id: string;
  hide?: boolean;
};

type DataType = {
  columns: SprintViewColumn[];
  filter: FilterGroup;
  groupBy?: GroupBy;
  sort?: Sort;
  header: {
  ...
  // Sprint-specific properties
  sprintDuration?: number; // Duration in days
  startDate?: string; // ISO date string
};
export type SprintViewData = BasicViewDataType<
  typeof sprintViewType.type,
  DataType
>;
export const sprintViewModel = sprintViewType.createModel<SprintViewData>({
  defaultName: 'Sprint View',
  dataViewManager: SprintSingleView,
  defaultData: viewManager => {
    ...
    return {
      ...
      groupProperties: [],
      sprintDuration: 14, // Default 2-week sprint
    };
  },
});
