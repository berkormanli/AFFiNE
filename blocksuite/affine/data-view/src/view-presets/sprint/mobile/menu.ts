...
export const popCardMenu = (
  ...
  groupKey: string,
  cardId: string,
  sprintViewLogic: MobileSprintViewUILogic
) => {
  const groupTrait = sprintViewLogic.view.traitGet(groupTraitKey);
  if (!groupTrait) {
    return;
  }
    ...
    menu.group({
      ...
      items: [
        menu.action({
          ...
          select: () => {
            sprintViewLogic.view.addCard(
              { before: true, id: cardId },
              groupKey
            );
            sprintViewLogic.ui$.value?.requestUpdate();
          },
        }),
        menu.action({
          name: 'Insert After',
    ...
    menu.group({
      items: [
        menu.action({
          ...
          prefix: DeleteIcon(),
          select: () => {
            sprintViewLogic.view.rowsDelete([cardId]);
            sprintViewLogic.ui$.value?.requestUpdate();
          },
        }),
      ],
